...
module.exports = function (opts) {
    ...
    return p
      ...
      .then(r => {
        t.equal(
          r.length,
          docs.length,
          'all, include_docs: false (promise mode)'
        )
      })
        ...
        return new Promise(function (resolve, reject) {
          p.all(function (err, r) {
            ...
            t.equal(
              r.length,
              docs.length,
              'same number of docs added come out! (cb mode)'
            )
            return resolve()
          })
        })
  ...
  test('bulkGet', t => {
    ...
    return Promise.resolve()
      ...
      .then(() =>
        dummyDocs.map(dummy => ({ _id: dummy._id, _rev: dummy._rev }))
      )
  ...
  test('update', function (t) {
    ...
    return p
      ...
      .then(function (doc) {
        ...
        return p.update(doc).then(function (updatedDoc) {
  ...
  test('dual callback/promise api', t => {
    setTimeout(() => {
      if (!succeeded) return t.end('dual api timeout')
    }, 50)
    var db = pouchyFactory({ name: 'testdb-dual' + Date.now() })
    var succeeded = false
    t.plan(1)
    return db.save({ from: 'promise' }).then(() => {
      return db.save({ from: 'cb' }, err => {
        if (err) return t.end(err)
        return db.all().then(docs => {
