...
var publicMethods = {
  ...
  all: function (opts, cb) {
    ...
    return this.db.allDocs(opts).then(function handleReceivedDocs (docs) {
      return docs.rows.reduce(function simplifyAllDocSet (r, v) {
        ...
        else if (opts.includeDesignDocs && doc._id.match(designDocRegex)) {
          r.push(doc)
        }
        return r
      }, [])
  ...
  bulkGet: function (opts, cb) {
    ...
    if (!Array.isArray(opts.docs)) {
      throw new Error(
        'bulkGet requires array of doc filters. please see couchdb bulkGet'
      )
    }
    ...
    return this.db.bulkGet(opts).then(function mapBulkGetResultsToRoot (r) {
  ...
  deleteAll: function (cb) {
    var deleteSingleDoc = function deleteSingleDoc (doc) {
  ...
  destroy: function () {
    ...
    if (this.syncEmitter) {
      if (this._replicationOpts && this._replicationOpts.live) {
        ...
        var isCompleteP = new Promise(
          function waitForLiveSyncComplete (resolve) {
  ...
  save: function (doc, opts, cb) {
    ...
    /* istanbul ignore next */
    var method =
      doc.hasOwnProperty('_id') && (doc._id || doc._id === 0) ? 'put' : 'post'
    return this.db[method](doc).then(function tidySaveResults (meta) {
      delete meta.status
      doc._id = meta.id
      doc._rev = meta.rev
      return doc
